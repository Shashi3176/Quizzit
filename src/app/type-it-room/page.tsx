...
export default function TypeItRoomPage() {
  const [rooms, setRooms] = useState<TypeItRoom[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRoom, setEditingRoom] = useState<TypeItRoom | null>(null);
  ...
  useEffect(() => {
    const fetchRooms = async () => {
        try {
          const response = await fetch('/api/type-it-room/list');
          if (response.ok) {
            const data = await response.json();
            setRooms(data);
          } else {
            toast.error('Failed to fetch type-it rooms.');
          }
        } catch (error) {
            console.error(error);
            toast.error('An error occurred while fetching type-it rooms.');
        }
    };
    fetchRooms();
  }, [version]);
  ...
  const handleEditRoom = useCallback((room: TypeItRoom) => {
    setEditingRoom(room);
    setIsModalOpen(true);
  }, []);
  ...
  const handleDeleteRoom = useCallback(async (id: number) => {
    if (window.confirm('Are you sure you want to delete this room?')) {
      try {
        const response = await fetch(`/api/type-it-room/${id}`, { method: 'DELETE' });
        if (response.ok) {
          toast.success('TypeIt room deleted successfully!');
          setVersion(v => v + 1);
        } else {
          const errorData = await response.json();
          toast.error(errorData.message || 'Failed to delete type-it room.');
        }
      } catch (error) {
        console.error(error);
        toast.error('An error occurred while deleting the type-it room.');
      }
    }
  }, []);
  ...
  const handleLockRoom = async (id: number) => {
    if (window.confirm('Are you sure you want to lock this room? Once locked, this action cannot be undone.')) {
        try {
          const response = await fetch(`/api/type-it-room/${id}/lock`, {
            method: 'PUT',
          ...
          if (response.ok) {
            toast.success('TypeIt room locked successfully!');
            setVersion(v => v + 1);
          } else {
            const errorData = await response.json();
            toast.error(errorData.message || 'Failed to lock type-it room.');
          }
        } catch (error) {
          console.error(error);
          toast.error('An error occurred while locking the type-it room.');
        }
    }
  };

  const handleSaveRoom = useCallback(async (roomData: Omit<TypeItRoom, 'id' | 'isLocked'>) => {
    const url = editingRoom ? `/api/type-it-room/${editingRoom.id}` : '/api/type-it-room';
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`TypeIt room ${editingRoom ? 'updated' : 'created'} successfully!`);
        setIsModalOpen(false);
        setVersion(v => v + 1);
      } else {
        const errorData = await response.json();
        toast.error(errorData.message || 'Failed to save type-it room.');
      }
    } catch (error) {
      console.error(error);
      toast.error('An error occurred while saving the type-it room.');
    }
  }, [editingRoom]);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-1">Create rooms where players type their answers</p>
          </div>
          
          <button
            onClick={handleAddRoom}
            className="group relative inline-flex items-center justify-center px-6 py-3 font-bold text-white transition-all duration-200 bg-gradient-to-r from-emerald-600 to-teal-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-600 hover:scale-105 hover:shadow-lg hover:shadow-emerald-500/25"
          >
        ...
        {rooms.length > 0 ? (
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-gray-800/50">
                    {room.isLocked ? (
                      <>
                        <button
                          onClick={() => router.push(`/type-it-room/${room.id}/start-quiz`)}
                      ...
                      <>
                        <button 
                          onClick={() => router.push(`/type-it-room/${room.id}`)} 
                          className="flex-1 py-2 px-3 text-sm bg-green-600/10 hover:bg-green-600/20 text-green-400 border border-green-500/20 rounded-lg transition-colors font-medium"
                        >
                          Manage
                        </button>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <h3 className="text-xl font-semibold text-gray-300 mb-2">No TypeIt Rooms Found</h3>
            <p className="text-gray-500 mb-6">Create a new type-it room to get started.</p>
            <button
              onClick={handleAddRoom}
...
interface TypeItRoomFormProps {
  room: TypeItRoom | null;
  onSave: (room: Omit<TypeItRoom, 'id' | 'isLocked'>) => void;
  onCancel: () => void;
}

function TypeItRoomForm({ room, onSave, onCancel }: TypeItRoomFormProps) {
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div>
          ...
          <input
            ...
            className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all placeholder-gray-600"
            placeholder="e.g. Spelling Bee Challenge"
            required
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all placeholder-gray-600 resize-none"
            rows={3}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 pr-12 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all placeholder-gray-600"
              />
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <button 
            type="submit" 
            className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 rounded-xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all hover:-translate-y-0.5"
          >
            Save Room
          </button>
        </div>
