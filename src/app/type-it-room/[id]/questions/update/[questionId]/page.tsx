...
export default function UpdateTypeItQuestionPage() {
  ...
  useEffect(() => {
    if (roomId && order) {
      fetch(`/api/type-it-room/${roomId}/questions/${order}`)
        .then((res) => res.json())
        .then((data) => {
          setText(data.text);
          setCorrectAnswers(data.correctAnswers && data.correctAnswers.length > 0 ? data.correctAnswers : ['']);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/type-it-room/${roomId}/questions/${order}`, {
        method: 'PUT',
        body: formData,
      });
      ...
      if (res.ok) {
        ...
        router.push(`/type-it-room/${roomId}/questions`);
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={addAnswer} className="text-xs font-bold text-yellow-400 hover:text-yellow-300 flex items-center gap-1 transition-colors">
                ...
                {correctAnswers.map((answer, index) => (
                  <div key={index} className="flex gap-2 group">
                    <div className="flex-grow relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-yellow-600 font-mono text-sm font-bold">{index + 1}</span>
                      <input
                        type="text"
                        value={answer}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-4 bg-[#0a0a0f] border border-gray-800 rounded-xl focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500 outline-none transition-all min-h-[80px] text-white"
              />
            </div>
