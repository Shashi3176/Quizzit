...
async function checkAuthorization(req: NextRequest, roomId: number) {
  try {
    ...
    const room = await prisma.the100Room.findUnique({
      where: { id: roomId },
...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    console.error('[GET /api/the-100/[id]] Auth error:', authErr);
    return NextResponse.json({ message: 'Authorization check failed' }, { status: 500 });
  }
  try {
    const room = await prisma.the100Room.findUnique({
      where: { id },
      include: { questions: { orderBy: { order: 'asc' } }, moderators: true, participants: true },
    });
    if (!room) return NextResponse.json({ message: 'Room not found' }, { status: 404 });
    const { user_password: _up, mod_password: _mp, ...details } = room;
    return NextResponse.json(details, { status: 200 });
  } catch (error) {
    console.error('[GET /api/the-100/[id]] Error:', error);
...
export async function PUT(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await prisma.the100Room.update({
      where: { id },
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (authError) return authError;
  try {
    await prisma.the100Room.delete({ where: { id } });
