...
export async function PUT(_req: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const room = await prisma.the100Room.findUnique({ where: { id } });
    if (!room) {
      return NextResponse.json({ message: "The-100 room not found" }, { status: 404 });
    }

    const newLockState = !room.isLocked;

    const updatedRoom = await prisma.the100Room.update({
      where: { id },
      data: {
