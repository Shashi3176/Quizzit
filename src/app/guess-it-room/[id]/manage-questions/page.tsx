...
// The main component for the manage questions page
export default function ManageGuessItQuestionsPage() {
  ...
  useEffect(() => {    
    ...
    const loadingTimeout = setTimeout(() => {
      setLoading(false);
      setAuthStatus('unauthenticated'); // Fall back to unauthenticated
    }, 10000); // 10 seconds timeout
    
    if (roomId) {
      fetchQuestions();
      
      // Fetch room details to check locked status
      fetch(`/api/guess-it-room/${roomId}`)
        .then(res => {
          if (!res.ok) {
    ...
    fetch('/api/auth/verifyToken')
      ...
      .then(data => {
        ...
        // API returns { message: "CGG", data: { decoded: ... } }
      ...
      .finally(() => {
        // Clear the timeout once auth check is complete
  ...
  const handleCreateQuestion = () => {
    ...
    router.push(`/guess-it-room/${roomId}/questions/create`);
  };

  const handleViewQuestion = (questionOrder: number) => {
    router.push(`/guess-it-room/${roomId}/questions/${questionOrder}`);
  };
  ...
  const handleUpdateQuestion = (questionOrder: number) => {
    ...
    router.push(`/guess-it-room/${roomId}/questions/update/${questionOrder}`);
  };
  ...
  const handleDeleteQuestion = async (questionOrder: number) => {
    ...
    if (window.confirm('Are you sure you want to delete this question?')) {
        try {
            const res = await fetch(`/api/guess-it-room/${roomId}/questions/${questionOrder}`, {
                method: 'DELETE',
            });
            if (res.ok) {
                fetchQuestions();
  ...
  // Show error if auth failed (but still show the page for viewing)
  if (error) {
    // If we have questions but also an error (e.g., room fetch failed), show the questions anyway
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-gray-100 p-4 sm:p-6 lg:p-8 font-sans selection:bg-purple-500/30">
      <div className="max-w-[1600px] mx-auto">
          ...
          <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
            {questions.length > 0 ? (
              questions.map((question) => (
                <div 
                  key={question.id} 
                  className="group bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-xl border border-gray-700/50 overflow-hidden hover:border-purple-500/30 transition-all duration-300"
                >
                  <div className="p-5">
                    <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                      ...
                      {user && (
                        <div className="flex items-center gap-2 flex-shrink-0">
                          <button 
                            onClick={() => handleViewQuestion(question.order)} 
                            className="inline-flex items-center gap-1.5 bg-blue-600/20 hover:bg-blue-600 border border-blue-500/30 hover:border-blue-500 text-blue-400 hover:text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all duration-200"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div className="py-16 text-center">
                ...
                <p className="text-gray-400 font-medium text-lg mb-2">No questions in this Guess It room yet.</p>
                <p className="text-gray-600 text-sm">Click "Create Question" to add your first question!</p>
                {user && !isRoomLocked && (
                  <button
                    onClick={handleCreateQuestion}
                    className="mt-6 inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white font-bold py-3 px-6 rounded-xl transition-all transform active:scale-95 shadow-lg shadow-purple-500/20"
                  >
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                    </svg>
        ...
        {/* Error Toast (if we have questions but also error) */}
