...
export default function StartQuizPage() {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${protocol}//${wsHost}/api/quiz-room/${roomId}/ws`;
                      ...
                      <div className="p-5">
                        ...
                        {question.options && question.options.length > 0 && (
                          <div className="mt-4 p-3 bg-gray-900/50 rounded-lg border border-gray-700/30">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">Options</p>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                              {question.options.map((option, index) => (
                                <div key={index} className="flex items-center gap-2 text-sm text-gray-400">
                                  <span className="flex-shrink-0 w-5 h-5 bg-gray-800 rounded flex items-center justify-center text-xs font-medium text-gray-500">
