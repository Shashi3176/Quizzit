...
const useWebSocket = (roomId: string) => {
  ...
  useEffect(() => {
    ...
    const wsUrl = `${protocol}//${wsHost}/api/quiz-room/${roomId}/ws${cookies ? `?token=${cookies}` : ''}`;
...
export default function LeaderboardPage() {
  ...
  const fetchAllQuestionsAnalytics = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/quiz-room/${roomId}/questions-analytics`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  const fetchPreviousQuestionAnalytics = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/quiz-room/${roomId}/previous-question-analytics`);
      if (!res.ok) return;
      const data = await res.json();
  ...
  const savePreviousQuestionAnalytics = useCallback(async (questionId: number, questionOrder: number, questionText: string, points: number) => {
    try {
      const res = await fetch(`/api/quiz-room/${roomId}/questions/${questionId}/analytics`);
      if (!res.ok) return;
      const data = await res.json();
      
      await fetch(`/api/quiz-room/${roomId}/previous-question-analytics`, {
        method: 'POST',
  ...
  const submitAnswer = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/quiz-room/${roomId}/questions/${currentQuestion.id}/answer`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ 
          ...
          {currentQuestion && (
            <div className="bg-[#12121b] rounded-2xl border border-purple-500/30 shadow-[0_0_50px_-12px_rgba(168,85,247,0.2)] overflow-hidden animate-slide-up">
              ...
              <div className="p-6">
                ...
                {isModerator ? (
                  /* Moderator: Show options as read-only */
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    {currentQuestion.options.map((option, index) => (
                      <div
                        key={index}
                        className="p-5 text-left rounded-xl border-2 bg-[#1a1a24] border-gray-700"
                      >
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                      {currentQuestion.options.map((option, index) => (
                        <button
                          ...
                          className={`p-5 text-left rounded-xl border-2 transition-all duration-200 ${
                            selectedAnswer === option 
                              ? 'bg-purple-600/20 border-purple-500 shadow-lg shadow-purple-500/20' 
                              : 'bg-[#1a1a24] border-gray-700 hover:border-purple-500/50 hover:bg-[#1f1f2e]'
                    ...
                    {!isSubmitted ? (
                      <button
                        ...
                        className="w-full bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 disabled:from-gray-700 disabled:to-gray-800 text-white font-bold py-4 px-6 rounded-xl transition-all transform active:scale-95 shadow-lg disabled:cursor-not-allowed disabled:opacity-40"
                      >
                        Submit Answer
                      </button>
                    ) : isSubmitting ? (
                      <div className="text-center py-6">
                        ...
                        <p className="text-purple-300 text-xl font-bold">Checking answer...</p>
                      </div>
                    ) : (
  ...
  if (loading && participants.length === 0) {
    return (
      <div className="min-h-screen bg-[#0a0a0f] flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-400 text-lg font-medium animate-pulse">Connecting to Quiz Server...</p>
      </div>
    );
  }
