...
export default function The100RoomPage() {
  const [rooms, setRooms] = useState<The100Room[]>([]);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingRoom, setEditingRoom] = useState<The100Room | null>(null);
  ...
  useEffect(() => {
    const fetchRooms = async () => {
      try {
        const response = await fetch('/api/the-100/list');
        if (response.ok) {
          const data = await response.json();
          setRooms(data);
        } else {
          toast.error('Failed to fetch The-100 rooms.');
        }
      } catch (error) {
        console.error(error);
        toast.error('An error occurred while fetching The-100 rooms.');
      }
    };
    fetchRooms();
  }, [version]);
  ...
  const handleEditRoom = useCallback((room: The100Room) => {
    setEditingRoom(room);
    setIsModalOpen(true);
  }, []);
  ...
  const handleDeleteRoom = useCallback(async (id: number) => {
    if (window.confirm('Are you sure you want to delete this room?')) {
      try {
        const response = await fetch(`/api/the-100/${id}`, { method: 'DELETE' });
        if (response.ok) {
          toast.success('The-100 room deleted successfully!');
          setVersion(v => v + 1);
        } else {
          const errorData = await response.json();
          toast.error(errorData.message || 'Failed to delete The-100 room.');
        }
      } catch (error) {
        console.error(error);
        toast.error('An error occurred while deleting the The-100 room.');
      }
    }
  }, []);
  ...
  const handleLockRoom = async (id: number) => {
    if (window.confirm('Are you sure you want to lock this room? Once locked, this action cannot be undone.')) {
      try {
        const response = await fetch(`/api/the-100/${id}/lock`, {
          method: 'PUT',
        ...
        if (response.ok) {
          toast.success('The-100 room locked successfully!');
          setVersion(v => v + 1);
        } else {
          const errorData = await response.json();
          toast.error(errorData.message || 'Failed to lock The-100 room.');
        }
      } catch (error) {
        console.error(error);
        toast.error('An error occurred while locking the The-100 room.');
      }
    }
  };

  const handleSaveRoom = useCallback(async (roomData: Omit<The100Room, 'id' | 'isLocked'>) => {
    const url = editingRoom ? `/api/the-100/${editingRoom.id}` : '/api/the-100';
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`The-100 room ${editingRoom ? 'updated' : 'created'} successfully!`);
        setIsModalOpen(false);
        setVersion(v => v + 1);
      } else {
        const errorData = await response.json();
        toast.error(errorData.message || 'Failed to save The-100 room.');
      }
    } catch (error) {
      console.error(error);
      toast.error('An error occurred while saving the The-100 room.');
    }
  }, [editingRoom]);
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-10">
          ...
          <button
            ...
            className="group relative inline-flex items-center justify-center px-6 py-3 font-bold text-white transition-all duration-200 bg-gradient-to-r from-orange-600 to-red-600 rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-600 hover:scale-105 hover:shadow-lg hover:shadow-orange-500/25"
          >
        ...
        {rooms.length > 0 ? (
                ...
                <div className="flex flex-col h-full">
                  ...
                  <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-gray-800/50">
                    {room.isLocked ? (
                      <>
                        <button
                          onClick={() => router.push(`/the-100/${room.id}`)}
                        ...
                        <button
                          onClick={() => router.push(`/the-100/${room.id}/start-quiz`)}
                      ...
                      <>
                        ...
                        <button 
                          ...
                          className="py-2 px-3 text-sm bg-orange-600/10 hover:bg-orange-600/20 text-orange-400 border border-orange-500/20 rounded-lg transition-colors"
                        >
                          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                          </svg>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-gray-500 mb-6">Create a new room to get started with your The-100 game.</p>
            <button
              onClick={handleAddRoom}
...
interface The100RoomFormProps {
  room: The100Room | null;
  onSave: (room: Omit<The100Room, 'id' | 'isLocked'>) => void;
  onCancel: () => void;
}

function The100RoomForm({ room, onSave, onCancel }: The100RoomFormProps) {
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
        <div>
          ...
          <input
            ...
            className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 transition-all placeholder-gray-600"
            placeholder="e.g. The Ultimate Challenge"
            required
          />
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 transition-all placeholder-gray-600 resize-none"
            rows={3}
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#0a0a0f] text-gray-200 border border-gray-700/50 rounded-xl py-2.5 px-4 pr-12 focus:outline-none focus:ring-2 focus:ring-orange-500/50 focus:border-orange-500/50 transition-all placeholder-gray-600"
              />
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <button 
            type="submit" 
            className="px-5 py-2.5 text-sm font-medium text-white bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 rounded-xl shadow-lg shadow-orange-500/20 hover:shadow-orange-500/40 transition-all hover:-translate-y-0.5"
          >
            Save Room
          </button>
        </div>
